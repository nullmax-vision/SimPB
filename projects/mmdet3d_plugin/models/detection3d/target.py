...
__all__ = ["SparseBox3DTarget", "SparseBox3DTargetWith2D"]
...
class SparseBox3DTarget(BaseTargetWithDenoising):
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_instance_id=None, metas=None):
        ...
        if gt_instance_id is not None:
            gt_instance_id = torch.stack([F.pad(x, (0, max_dn_gt - x.shape[0]), value=-1) for x in gt_instance_id])
        ...
        if self.add_neg_dn:
            ...
            flag = torch.where(torch.rand_like(box_target) > 0.5,
                               noise_neg.new_tensor(1), noise_neg.new_tensor(-1))
            noise_neg *= flag
        ...
        dn_anchor = dn_anchor.reshape(self.num_dn_groups, bs, num_gt, state_dims).permute(1, 0, 2, 3).flatten(1, 2)
        dn_box_target = dn_box_target.reshape(self.num_dn_groups, bs, num_gt, state_dims).permute(1, 0, 2, 3).flatten(1, 2)
        dn_cls_target = dn_cls_target.reshape(self.num_dn_groups, bs, num_gt).permute(1, 0, 2).flatten(1)

        if gt_instance_id is not None:
            dn_id_target = dn_id_target.reshape(self.num_dn_groups, bs, num_gt).permute(1, 0, 2).flatten(1)
        else:
            dn_id_target = None
        ...
        if self.add_neg_dn:
            cls_target = torch.cat([cls_target, cls_target], dim=1).reshape(
                self.num_dn_groups, bs, num_gt).permute(1, 0, 2).flatten(1)
            # valid denotes the items is not from pad.
            valid_mask = torch.logical_or(valid_mask, ((cls_target >= 0) & (dn_cls_target == -3)))
    ...
    def update_dn(
        ...
        dn_metas = [dn_feat, dn_anchor, dn_reg_target,
                    dn_cls_target, valid_mask, dn_id]
        temp_dn_metas = [temp_dn_feat, temp_dn_anchor, temp_reg_target,
                         temp_cls_target, temp_valid_mask, temp_dn_id]
        output = []
        for i, (meta, temp_meta) in enumerate(zip(dn_metas, temp_dn_metas)):
...
@BBOX_SAMPLERS.register_module()
class SparseBox3DTargetWith2D(BaseTargetWithDenoising):
    def __init__(
        self,
        cls_weight=2.0,
        alpha=0.25,
        ...
        add_neg_dn=True,
        num_temp_dn_groups=0,
        with_alpha_angle=False,
        with_depth2d=False,
    ):
        super(SparseBox3DTargetWith2D, self).__init__(
            num_dn_groups, num_temp_dn_groups
        )
        self.cls_weight = cls_weight
        self.box_weight = box_weight
        ...
        self.max_dn_gt = max_dn_gt
        self.add_neg_dn = add_neg_dn
        self.with_alpha_angle = with_alpha_angle
        self.with_depth2d = with_depth2d
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_instance_id=None, metas=None):
        ...
        batch_size, num_gt, state_dims = box_target.shape

        # 2d gt
        gt_labels_2d = metas['gt_labels_2d']
        gt_bboxes_2d = metas['gt_bboxes_2d']
        gt_2d_3d_map = metas['gt_2d_3d_map']
        gt_alphas_2d = metas['gt_alphas_2d'] if self.with_alpha_angle else None
        gt_depths_2d = metas['gt_depths_2d'] if self.with_depth2d else None

        num_cams = len(gt_labels_2d[0])

        trans_mask2d = cls_target.new_zeros((batch_size, max_dn_gt, num_cams)).to(torch.bool)
        cls_target2d = cls_target.new_ones((batch_size, num_gt, num_cams), dtype=torch.long) * -1
        box_target2d = cls_target.new_zeros((batch_size, num_gt, num_cams, 4), dtype=torch.float)
        alpha_target2d = cls_target.new_zeros((batch_size, num_gt, num_cams), dtype=torch.float) if self.with_alpha_angle else None
        depth_target2d = cls_target.new_zeros((batch_size, num_gt, num_cams), dtype=torch.float) if self.with_depth2d else None

        for bs in range(batch_size):
            for nc in range(num_cams):
                indexes3d = gt_2d_3d_map[bs][nc]
                index_mask = indexes3d < max_dn_gt
                indexes3d = indexes3d[index_mask]
                trans_mask2d[bs, indexes3d, nc] = True
                box_target2d[bs, indexes3d, nc] = gt_bboxes_2d[bs][nc][index_mask]
                cls_target2d[bs, indexes3d, nc] = gt_labels_2d[bs][nc][index_mask]
                if self.with_alpha_angle:
                    alpha_target2d[bs, indexes3d, nc] = gt_alphas_2d[bs][nc][index_mask]
                if self.with_depth2d:
                    depth_target2d[bs, indexes3d, nc] = gt_depths_2d[bs][nc][index_mask]
        ...
        if self.num_dn_groups > 1:
            ...
            if gt_instance_id is not None:
                gt_instance_id = gt_instance_id.tile(self.num_dn_groups, 1)
            # repeat 2d
            trans_mask2d = trans_mask2d.tile(self.num_dn_groups, 1, 1)
            cls_target2d = cls_target2d.tile(self.num_dn_groups, 1, 1)
            box_target2d = box_target2d.tile(self.num_dn_groups, 1, 1, 1)
            if alpha_target2d is not None:
                alpha_target2d = alpha_target2d.tile(self.num_dn_groups, 1, 1)
            if depth_target2d is not None:
                depth_target2d = depth_target2d.tile(self.num_dn_groups, 1, 1)
        ...
        # construct target for supervising
        dn_box_target = torch.zeros_like(dn_anchor)
        dn_cls_target = -torch.ones_like(cls_target) * 3
        if gt_instance_id is not None:
            dn_id_target = -torch.ones_like(gt_instance_id)

        dn_trans_mask2d = trans_mask2d.clone()
        dn_box_target2d = torch.zeros_like(box_target2d)
        dn_cls_target2d = -torch.ones_like(cls_target2d) * 4
        if alpha_target2d is not None:
            dn_alpha_target2d = torch.zeros_like(alpha_target2d)
        if depth_target2d is not None:
            dn_depth_target2d = torch.zeros_like(depth_target2d)
        ...
        if self.add_neg_dn:
            ...
            dn_trans_mask2d = torch.cat([dn_trans_mask2d, dn_trans_mask2d], dim=1)
            dn_cls_target2d = torch.cat([dn_cls_target2d, dn_cls_target2d], dim=1)
            dn_box_target2d = torch.cat([dn_box_target2d, dn_box_target2d], dim=1)
            if alpha_target2d is not None:
                dn_alpha_target2d = torch.cat([dn_alpha_target2d, dn_alpha_target2d], dim=1)
            if depth_target2d is not None:
                dn_depth_target2d = torch.cat([dn_depth_target2d, dn_depth_target2d], dim=1)
        ...
        for i in range(dn_anchor.shape[0]):
            ...
            dn_trans_mask2d[i, anchor_idx] = trans_mask2d[i, gt_idx]
            dn_cls_target2d[i, anchor_idx] = cls_target2d[i, gt_idx]
            dn_box_target2d[i, anchor_idx] = box_target2d[i, gt_idx]
            if alpha_target2d is not None:
                dn_alpha_target2d[i, anchor_idx] = alpha_target2d[i, gt_idx]
            if depth_target2d is not None:
                dn_depth_target2d[i, anchor_idx] = depth_target2d[i, gt_idx]


        dn_anchor = dn_anchor.reshape(self.num_dn_groups, batch_size, num_gt, state_dims).permute(1, 0, 2, 3).flatten(1, 2)
        dn_box_target = dn_box_target.reshape(self.num_dn_groups, batch_size, num_gt, state_dims).permute(1, 0, 2, 3).flatten(1, 2)
        dn_cls_target = dn_cls_target.reshape(self.num_dn_groups, batch_size, num_gt).permute(1, 0, 2).flatten(1)
        if gt_instance_id is not None:
            dn_id_target = dn_id_target.reshape(self.num_dn_groups, batch_size, num_gt).permute(1, 0, 2).flatten(1)
        else:
            dn_id_target = None

        dn_trans_mask2d = dn_trans_mask2d.reshape(self.num_dn_groups, batch_size, num_gt, num_cams).permute(1, 0, 2, 3).flatten(1, 2)
        dn_cls_target2d = dn_cls_target2d.reshape(self.num_dn_groups, batch_size, num_gt, num_cams).permute(1, 0, 2, 3).flatten(1, 2)
        dn_box_target2d = dn_box_target2d.reshape(self.num_dn_groups, batch_size, num_gt, num_cams, 4).permute(1, 0, 2, 3, 4).flatten(1, 2)
        dn_alpha_target2d = dn_alpha_target2d.reshape(self.num_dn_groups, batch_size, num_gt, num_cams).permute(1, 0, 2, 3).flatten(1, 2) if alpha_target2d is not None else None
        dn_depth_target2d = dn_depth_target2d.reshape(self.num_dn_groups, batch_size, num_gt, num_cams).permute(1, 0, 2, 3).flatten(1, 2) if depth_target2d is not None else None

        # ======================
        valid_mask = dn_cls_target >= 0
        valid_mask2d = dn_cls_target2d >= 0
        if self.add_neg_dn:
            cls_target = torch.cat([cls_target, cls_target], dim=1).reshape(
                self.num_dn_groups, batch_size, num_gt).permute(1, 0, 2).flatten(1)
            # valid denotes the items is not from pad.
            valid_mask = torch.logical_or(valid_mask, ((cls_target >= 0) & (dn_cls_target == -3)))

            cls_target2d = torch.cat([cls_target2d, cls_target2d], dim=1).reshape(
                self.num_dn_groups, batch_size, num_gt, num_cams).permute(1, 0, 2, 3).flatten(1, 2)
            dn_valid_mask2d = torch.logical_or(valid_mask2d, ((cls_target2d >= 0) & (dn_cls_target2d == -4)))
        ...
        attn_mask = attn_mask == 1
        dn_cls_target = dn_cls_target.long()
        dn_cls_target2d = dn_cls_target2d.long()
    ...
    def update_dn(
        ...
        num_noraml_anchor,
        temporal_valid_mask,
        dn_trans_mask2d=None,
        dn_valid_mask2d=None,
        dn_cls_target2d=None,
        dn_box_target2d=None,
        ...
        num_dn_groups = self.num_dn_groups
        num_dn = num_dn // num_dn_groups
        num_cams = dn_cls_target2d.size(-1)
        ...
        dn_trans_mask2d = dn_trans_mask2d.reshape(bs, num_dn_groups, num_dn, num_cams)
        dn_valid_mask2d = dn_valid_mask2d.reshape(bs, num_dn_groups, num_dn, num_cams)
        dn_cls_target2d = dn_cls_target2d.reshape(bs, num_dn_groups, num_dn, num_cams)
        dn_box_target2d = dn_box_target2d.reshape(bs, num_dn_groups, num_dn, num_cams, -1)
        dn_alpha_target2d = dn_alpha_target2d.reshape(bs, num_dn_groups, num_dn, num_cams) if dn_alpha_target2d is not None else None
        dn_depth_target2d = dn_depth_target2d.reshape(bs, num_dn_groups, num_dn, num_cams) if dn_depth_target2d is not None else None
        ...
        # bs, num_temp_dn_groups, num_temp_dn, num_dn
        match = temp_dn_id[..., None] == dn_id[:, :num_temp_dn_groups, None]
        no_match_mask = torch.all(torch.logical_not(match), dim=-1)
        ...
        temp_trans_mask2d = torch.where(
            no_match_mask[..., None].tile(1, 1, 1, num_cams),
            self.dn_metas["dn_trans_mask2d"].new_tensor(0),
            self.dn_metas["dn_trans_mask2d"],
        )
        temp_valid_mask2d = torch.where(
            no_match_mask[..., None].tile(1, 1, 1, num_cams),
            self.dn_metas["dn_valid_mask2d"].new_tensor(0),
            self.dn_metas["dn_valid_mask2d"],
        )
        temp_cls_target2d = torch.where(
            no_match_mask[..., None].tile(1, 1, 1, num_cams),
            self.dn_metas["dn_cls_target2d"].new_tensor(-1),
            self.dn_metas["dn_cls_target2d"],
        )
        temp_alpha_target2d = torch.where(
            no_match_mask[..., None].tile(1, 1, 1, num_cams),
            self.dn_metas["dn_alpha_target2d"].new_tensor(0),
            self.dn_metas["dn_alpha_target2d"],
        ) if dn_alpha_target2d is not None else None
        temp_depth_target2d = torch.where(
            no_match_mask[..., None].tile(1, 1, 1, num_cams),
            self.dn_metas["dn_depth_target2d"].new_tensor(0),
            self.dn_metas["dn_depth_target2d"],
        ) if dn_depth_target2d is not None else None
        temp_box_target2d = (match[..., None, None] * dn_box_target2d[:, :num_temp_dn_groups, None]).sum(dim=3)

        # handle the misalignment the length of temp_dn to dn caused by the
        # change of num_gt, then concat the temp_dn and dn
        dn_metas = [dn_feat, dn_anchor, dn_reg_target, dn_cls_target, valid_mask, dn_id,
                    dn_trans_mask2d, dn_valid_mask2d, dn_cls_target2d, dn_box_target2d, dn_alpha_target2d, dn_depth_target2d]
        temp_dn_metas = [temp_dn_feat, temp_dn_anchor, temp_reg_target, temp_cls_target, temp_valid_mask, temp_dn_id,
                         temp_trans_mask2d, temp_valid_mask2d, temp_cls_target2d, temp_box_target2d, temp_alpha_target2d, temp_depth_target2d]
        output = []
        for i, (meta, temp_meta) in enumerate(zip(dn_metas, temp_dn_metas)):
            if meta is None and temp_meta is None:
                output.append(None)
                continue
    ...
    def cache_dn(
        ...
        dn_anchor = dn_anchor.detach().reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask]
        dn_cls_target = dn_cls_target.reshape(bs, num_dn_groups, num_temp_dn)[:, temp_group_mask]
        dn_trans_mask2d = dn_trans_mask2d.reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask]
        dn_valid_mask2d = dn_valid_mask2d.reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask]
        dn_cls_target2d = dn_cls_target2d.reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask]
        dn_alpha_target2d = dn_alpha_target2d.reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask] if dn_alpha_target2d is not None else None
        dn_depth_target2d = dn_depth_target2d.reshape(bs, num_dn_groups, num_temp_dn, -1)[:, temp_group_mask] if dn_depth_target2d is not None else None
        ...
        self.dn_metas = dict(
            ...
            valid_mask=valid_mask,
            dn_id_target=dn_id_target,
            dn_trans_mask2d=dn_trans_mask2d,
            dn_valid_mask2d=dn_valid_mask2d,
            dn_cls_target2d=dn_cls_target2d,
            dn_alpha_target2d=dn_alpha_target2d,
            dn_depth_target2d=dn_depth_target2d,
        )