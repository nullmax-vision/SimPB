...
def draw_image_bbox2d_on_img(
    bboxes2d, raw_img, img_metas=None, color=(0, 255, 0), thickness=1
):
    for i, bbox2d in enumerate(bboxes2d):
        bbox = np.array(bbox2d)
        raw_img = cv2.rectangle(raw_img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), color, thickness)
    return raw_img
...
def draw_lidar_bbox3d_on_bev(bboxes_3d, bev_size, bev_range=115, color=(255, 0, 0), thickness=3, bev_img=None):
    ...
    # init bev image
    if bev_img is None:
        bev = np.ones([bev_h, bev_w, 3]) * 255

        marking_color = (110, 110, 110)

        for cir in range(int(bev_range / 20)):
            cv2.circle(bev, (int(bev_h / 2), int(bev_w / 2)),
                       int((cir + 1) * 10 / bev_resolution), marking_color, thickness=thickness)
        cv2.line(bev, (0, int(bev_h / 2)), (bev_w, int(bev_h / 2)), marking_color, thickness=thickness-1)
        cv2.line(bev, (int(bev_w / 2), 0), (int(bev_w / 2), bev_h), marking_color, thickness=thickness-1)
    else:
        bev = bev_img
