...
def filter_info2d(input_dict, mask):
    if isinstance(mask, torch.Tensor):
        mask = mask.numpy()

    maskes_2d = [np.ones(len(x), dtype=np.bool) for x in input_dict['gt_bboxes_2d']]
    for cam_idx, map_2d_3d in enumerate(input_dict['gt_2d_3d_map']):
    ...
    for cam_idx, mask_2d in enumerate(maskes_2d):
        input_dict['gt_bboxes_2d'][cam_idx] = input_dict['gt_bboxes_2d'][cam_idx][mask_2d]
...
class NuScenesSparse4DAdaptor(object):
    ...
    def __call__(self, input_dict):
        ...
        for key in ['gt_bboxes_2d', 'gt_labels_2d', 'gt_centers_2d',
                    'gt_depths_2d', 'gt_2d_3d_map', 'gt_alphas_2d']:
            if key not in input_dict:
                continue
            if isinstance(input_dict[key], list):
                input_dict[key] = DC([to_tensor(res) for res in input_dict[key]])
            else:
                input_dict[key] = DC(to_tensor(input_dict[key]))

        imgs = [img.transpose(2, 0, 1) for img in input_dict["img"]]
        imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
        input_dict["img"] = DC(to_tensor(imgs), stack=True)

        input_dict["intrinsics"] = np.float32(np.stack(input_dict["intrinsics"]))
        input_dict["extrinsics"] = np.float32(np.stack(input_dict["extrinsics"]))
...
class InstanceNameFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        if 'gt_bboxes_2d' in input_dict:
            input_dict = filter_info2d(input_dict, gt_bboxes_mask)
...
class CircleObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        # update 2d labels
        if 'gt_bboxes_2d' in input_dict:
            input_dict = filter_info2d(input_dict, mask)
